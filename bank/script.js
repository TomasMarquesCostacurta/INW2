class Conta { constructor(numero, cpf, saldo, ativo) { this.numero = numero; this.cpf = cpf; this.saldo = saldo; this.ativo = ativo } 
...
debito(valor) { 
    ...
    desativar(){ 
        ...
        } else { console.log("A conta já está desativa") } } 
    }
class corrente extends conta{ 
    constructor(numero, cpf, saldo, ativo,Cheque){ 
        ...
        EmitirCheque(){ 
            ...
            else if (this.saldo < 30){ console.log("Saldo insuficiente") }
             else{ console.log("Você comprou um talão de cheque."); this.saldo -= 30 } } 
            }
...
class especial extends conta { 
    ...
    { super(numero, cpf, saldo, ativo) 
        ...
        usarLimite(valor) { 
            if (valor < 0) { 
                console.log("impossivel realizar valor negativo...") } 
                else if (valor == 0) { console.log("impossivel realizar valor zerado...") } 
                else if (valor > (this.saldo + this.limite)) { console.log("impossivel realizar sem valor no limite do saldo...") } 
...
if (opcao == "1") { 
    ...
    for(let x=1;x<=10;x++){
         ...
         if(op =="D"){ cp1.debito(valor) } 
         else if(op =="C"){ cp1.credito(valor) } 
    ...
    for( x=1;x<=10;x++){ 
        valor = parseFloat(leia("Digite o valor: "))
         op = leia("Digite D de debito ou C de credito:") 
         if(op =="D" ){ cc1.debito(valor) } 
         else if(op =="C"){ cc1.credito(valor) } console.log("Saldo atual R$:" +cc1.saldo) 
         op = leia("Gostaria de encerrar a transacao?") 
         if(op == "S"){ op = leia("Gostaria de um cheque?") 
         if(op == "S"){ cc1.EmitirCheque(); } cc1.desativar() 
         break } } }        
         else if (opcao == "3") { 
            let numero = leia("Digite o numero da conta:") 
            let cpf = leia("Digite o cpf da conta:") 
            ...
            for(let x=1; x<=10; x++){
                 ...
                 op = leia("Digite D para debito ou C para credito: ") 
                  if(op=="D"){ 
                    cee1.debito(valor) } 
                    else if(op=="C"){ cee1.credito(valor) } 
 ...
 for (let x = 1; x <= 10; x++) { 
    ...
    if (op == "D") { valor = parseInt(leia("digite o valor para debito :")) 
    c1.debito(valor) } 
    else if (op == "C") { valor = parseInt(leia("digite o valor para credito :")) 
    c1.credito(valor) } console.log("saldo atual da conta:" + c1.saldo) } 
